...
#SBATCH --job-name=snow_maskflownet_i30
#SBATCH --output=slurm/snow_maskflownet_i30_%j.out
#SBATCH --cpus-per-task=16
#SBATCH --gres=gpu:1
#SBATCH --partition=gpu_4_a100
#SBATCH --error=slurm/snow_maskflownet_i30_%j.err
        ...
        do
            ...
            then
                if [[ $VAL_DATASET == "sintel-clean-val" ]]; then
                    OUTPUT_PATH="outputs/validate/snow/clean/i30"
                elif [[ $VAL_DATASET == "sintel-final-val" ]]; then
                    OUTPUT_PATH="outputs/validate/snow/final/i30"
                fi
