...
#SBATCH --job-name=kitti_missing_two
#SBATCH --output=slurm/kitti_missing_iterations_two_%A_%a.out
#SBATCH --error=slurm/kitti_missing_iterations_two_err_%A_%a.out
#SBATCH --array=0-7%4

models="raft gma rpknet ccmr craft csflow dicl dip fastflownet maskflownet flow1d flowformer flowformer++ gmflow gmflownet hd3 irr_pwc liteflownet liteflownet2 liteflownet3_pseudoreg llaflow matchflow ms_raft+ rapidflow scopeflow scv4 separableflow skflow starflow videoflow_bof"
dataset="kitti-2015"
checkpoint="kitti"
targeteds="False"
targets="negative"
