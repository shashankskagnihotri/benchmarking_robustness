...
from attacks.help_function import datasets
from attacks.help_function.config_specs import Paths, Conf
#from DistractingDownpour.flow_library.flow_plot import colorplot_light
...
def import_and_load(net='RAFT', make_unit_input=False, variable_change=False, device=torch.device("cpu"), make_scaled_input_weather_model=False, **kwargs):
    ...
    if make_scaled_input_weather_model:
        ...
        try:
            if net == 'RAFT':
                from DistractingDownpour.models.raft.raft import RAFT

                # set the path to the corresponding weights for initializing the model
                path_weights = custom_weight_path or 'benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_pretrained_weights/raft-sintel.pth'

                # possible adjustements to the config can be made in the file
                # found under models/_config/raft_config.json
                with open("benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_config/raft_config.json") as file:
                ...
                from DistractingDownpour.models.gma.network import RAFTGMA

                # set the path to the corresponding weights for initializing the model
                path_weights = custom_weight_path or '/pfs/work7/workspace/scratch/ma_xinygao-team_project_fss2024/benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_pretrained_weights/gma-sintel.pth'

                # possible adjustements to the config file can be made
                # under models/_config/gma_config.json
                with open("/pfs/work7/workspace/scratch/ma_xinygao-team_project_fss2024/benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_config/gma_config.json") as file:
                    config = json.load(file)
                ...
                from DistractingDownpour.models.FlowFormer.core.FlowFormer import build_flowformer
                from DistractingDownpour.models.FlowFormer.configs.things_eval import get_cfg as get_things_cfg

                path_weights = custom_weight_path or 'benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_pretrained_weights/flowformer_weights/sintel.pth'
                cfg = get_things_cfg()
                ...
                from DistractingDownpour.models.SpyNet.SpyNet import Network as SpyNet
                ...
                if net == "FlowNet2":
                    from DistractingDownpour.models.FlowNet.FlowNet2 import FlowNet2
                    # set path to pretrained weights
                    path_weights = custom_weight_path or 'benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_pretrained_weights/FlowNet2_checkpoint.pth.tar'
                ...
                from DistractingDownpour.models.FlowNetCRobust.FlowNetC_flexible_larger_field import FlowNetC_flexible_larger_field

                # initialize model and load pretrained weights
                path_weights = custom_weight_path or 'benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_pretrained_weights/RobustFlowNetC.pth'
                ...
                if net == "FlowNetC":
                    from DistractingDownpour.models.FlowNet.FlowNetC import FlowNetC
                    # set path to pretrained weights
                    path_weights = custom_weight_path or 'benchmarking_robustness/optical_flow_estimation/DistractingDownpour/models/_pretrained_weights/flownetc_EPE1.766.tar'
...
def prepare_dataloader(attack_args, shuffle=False, batch_size=1, get_weather=False):
    """Get a PyTorch dataloader for the specified dataset
    """
    if attack_args["weather_dataset"] == 'Sintel':
        if attack_args["weather_dataset_stage"] == 'training':
            dataset = datasets.MpiSintel(attack_args, split=Paths.splits("sintel_train"),
                root=Paths.config("sintel_mpi"), has_gt=True, get_weather=get_weather)
        elif attack_args["weather_dataset_stage"] == 'evaluation':
            # with this option, ground truth and valid are None!!
            dataset = datasets.MpiSintel(attack_args, split=Paths.splits("sintel_eval"),
                root=Paths.config("sintel_mpi"), has_gt=False, get_weather=get_weather)
        else:
            raise ValueError(f'The specified mode: the data_stage is unknown.')
    else:
        raise ValueError("Unknown dataset %s, use 'Sintel'." )
    ...
    if attack_args["weather_small_run"]:
...
#     return float_val
     ...
     # float_val = torch_float.detach().cpu().numpy().item()
