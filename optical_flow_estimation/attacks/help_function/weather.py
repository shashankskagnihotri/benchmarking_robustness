...
def get_weather(has_weather, weatherdat, scene_data, attack_args, seed=None, load_only=False):
    if has_weather:
        print("Loading existing weather model instead of generating particles.")
        weather, success = utils.load_weather(weatherdat)
        if success:
          if attack_args["weather_recolor"]:
            weather = recolor_weather(weather, attack_args)
    ...
    if not has_weather or not success:
        weather = generate_weather(scene_data, attack_args, seed=seed)
    return weather
