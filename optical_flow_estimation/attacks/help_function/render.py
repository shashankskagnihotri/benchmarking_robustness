...
def scat_mult(img, points, factors):
    ...
    indices = indices.unsqueeze(1).repeat(1, 3)
    img_flat = img.reshape(-1, 3)
    img_flat = img_flat.scatter_reduce(
        0, indices, factors, reduce="prod"
    )  # the backward pass for .scatter_()/.scatter() (in/out of place) is not implemented!
    img2 = img_flat.reshape(h, w, 3)
    return img2
...
def bilinear_interpolation(d0, d1, V00, V01, V10, V11, transp=None):
    ...
    return (
        d1[..., 0] * d1[..., 1] * V00 * transp
        + d0[..., 1] * d1[..., 0] * V01 * transp
        + d0[..., 0] * d1[..., 1] * V10 * transp
        + d0[..., 0] * d0[..., 1] * V11 * transp
    )
...
def twoD_to_img(
    image, image_depth, point2d, flakes, flakes_color, flakes_transp, flakes_depth, args
):
    ...
    image = F.pad(
        image, (img_pad, img_pad, img_pad, img_pad), "constant", 0
    )  # image padding, img_pad because flakes are padded by 2 in every direction
    image_depth = F.pad(
        image_depth, (img_pad + 1, img_pad + 1, img_pad + 1, img_pad + 1), "replicate"
    )  # depth padding, 1 larger than image padding because depth needs 1 additonal layer due to depth interpolation to flake position.
    ...
    flake_interpol_result = bilinear_interpolation_new(
        d0,
        d1,
        flake_interpol_TL,
        flake_interpol_TR,
    ...
    y, x = torch.meshgrid(
        torch.arange(2 * flake_pad_half, device=V1.device),
        torch.arange(2 * flake_pad_half, device=V1.device),
    )
    pos = torch.cat((x[None], y[None]), dim=0)

    # DEPTH-AWARENESS
    if args["weather_depth_check"]:
        ...
        grid = torch.reshape(
            grid, (grid.shape[0], gridsize_1 * gridsize_2, grid.shape[3], grid.shape[4])
        )
        ...
        depth_samples = torch.reshape(
            depth_samples, (grid.shape[0], gridsize_1, gridsize_2, grid.shape[2])
        )
        ...
        if args["weather_depth_check_differentiable"]:
            factor = 30
        else:
            factor = 250
        # sharp sigmoid instead of step function - the factor should for differentiability, but large for visual effects.
        flake_depths = 1 / (
            1 + torch.exp(factor * (flakes_depth[..., None, None] - depth_samples))
        )

        if not args["weather_depth_check_differentiable"]:
    ...
    flake_interpol_result_alpha = torch.unsqueeze(
        flake_interpol_result[..., 3, :, :], 2
    )

    # Then apply alpha to color channels in linear color space
    flake_interpol_result = (
        flakes_color[..., None, None] + flake_interpol_result_color_offsets
    ) * flake_interpol_result_alpha
    ...
    flk_transp = (
        args["weather_transparency_scale"] * 1.0 / 2.0 * (torch.tanh(flakes_transp) + 1)
    )  # shape: (batchsize, numflakes)
    flk_transp = (
        torch.ones_like(flake_interpol_result.permute(0, 1, 3, 4, 2))
        * flk_transp[..., None, None, None]
    )
    flk_transp = flk_transp.reshape(batchsize, -1, 3)
    flk_alpha = torch.ones_like(
        flake_interpol_result.permute(0, 1, 3, 4, 2)
    ) * flake_interpol_result_alpha.permute(0, 1, 3, 4, 2)
    flk_alpha = flk_alpha.reshape(batchsize, -1, 3)

    if args["weather_rendering_method"] == "meshkin":
        # For method details see related work in: https://jcgt.org/published/0002/02/09/
        sum_ai = add_things_on_img(
            torch.zeros_like(image[0].permute(1, 2, 0)),
            pos[0],
            flk_transp[0] * flk_alpha[0],
        )  # overall transparency = flake_alpha * flake_transp
        sum_ci = add_things_on_img(
            torch.zeros_like(image[0].permute(1, 2, 0)),
            pos[0],
            flk_transp[0] * flk_color[0],
        )  # part of overall transparency (the alpha) is implicitely in "flk_color" (via flake_interpol_result) and for a full transparency-premultiplication we also need the global transparency attenuation flk_transp.

        image = sum_ci + image[0].permute(1, 2, 0) * (
            1 - sum_ai
        )  # Meshkins Method -> needs sum_ci (color*transp) because otherwise (color*transp*alpha) artifacts exist around the particles in the background
    elif args["weather_rendering_method"] == "additive":
        image = add_things_on_img(
            image[0].permute(1, 2, 0), pos[0], flk_transp[0] * flk_color[0]
        )
    else:
        raise RuntimeWarning(
            f"Unknown transparency method rendering_method. Returning image without particles"
        )
    ...
    # UNPADDING
    image = image[
        ..., img_pad : image_h + img_pad, img_pad : image_w + img_pad
    ]  # image unpadding, flake_size+4 because flakes are padded by 2 in every direction
...
def render(image1, image2, scene_data, weather, args):
    ...
    print("motion", args["weather_do_motionblur"])
    if args["weather_do_motionblur"]:
        samples = args["weather_motionblur_samples"]
    else:
        samples = 1
    ...
    for i in range(samples):
        ...
        if samples > 1:
            flakes_transp_ = (
                1.0 / 2.0 * (torch.tanh(flakes_transp) + 1) / samples
            )  # transformation to linear scale and divide by samples
            flakes_transp_ = torch.atanh(
                2.0 * flakes_transp_ - 1
            )  # flakes_transp_ = backtransform to -infty ... infty
            # flakes_transp_ = (flakes_transp_ + 2.) / samples - 2.

            # Add new flake-3D positions in direction of motion blur
            offs = torch.tensor(
                [i / (samples - 1) * args["weather_motionblur_scale"]]
            )  # goes from (0 ... 1)*motionblur_scale
...
def renderimg(
    image, full_P, points_3d, flakes, flakes_color, flakes_transp, image_depth, args
):
