...
def twoD_to_img(image, image_depth, point2d, flakes, flakes_color, flakes_transp, flakes_depth, args):
    ...
    flk_transp = args["weather_transparency_scale"] * 1./2. * (torch.tanh(flakes_transp) + 1) # shape: (batchsize, numflakes)
    ...
    if args["weather_rendering_method"] == "meshkin":
        ...
        raise RuntimeWarning(f"Unknown transparency method rendering_method. Returning image without particles")
...
def render(image1, image2, scene_data, weather, args):
    ...
    for i in range(samples):
        ...
        if samples > 1:
            ...
            offs = torch.tensor([i / (samples-1) * args["weather_motionblur_scale"]]) # goes from (0 ... 1)*motionblur_scale
