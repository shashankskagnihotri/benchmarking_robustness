...
from attacks.help_function import frame_utils
...
class MpiSintel(FlowDataset):
    def __init__(self, args, aug_params=None, split=Paths.splits("sintel_train"), root=Paths.config("sintel_mpi"), has_gt=False, get_weather=False):
        ...
        image_root = osp.join(root, split, args["weather_dstype"])
        ...
        all_sc = args["weather_single_scene"] == '' and args["weather_from_scene"] == ''


        for scene in sorted(os.listdir(image_root)):
            if all_sc or (all_scenes_from_here or scene in [args["weather_single_scene"], args["weather_from_scene"]]):
                if scene == args["weather_from_scene"]:
                    all_scenes_from_here = True
                image_list = sorted(glob(osp.join(image_root, scene, '*.png')))
                # check if dataset should be shortened
                if args["weather_frame_per_scene"] == 0:
                    no_imges = len(image_list)-1
                else:
                    no_imges = args["weather_frame_per_scene"]
                for i in range(len(image_list)-1):
                    if i >= args["weather_from_frame"] and i < args["weather_from_frame"]+no_imges:
                      ...
                      if self.has_camdata and self.get_weather:
                        ...
                        #   print("文件位置：", os.path.join(depth_root, f"{scene}/frame_{i+2:04d}.dpt"))
                         # print("第二个深度值为：", gt2_depth)

                          gt2_depth = torch.unsqueeze(torch.from_numpy(gt2_depth), dim=0)

                          gt1_depth, gt2_depth, rel_mat = rescale_sintel_scenes(scene, gt1_depth, gt2_depth, rel_mat, OVERALL_SCALE=args["weather_scene_scale"])
                ...
                if split != 'test':
                    self.flow_list += sorted(glob(osp.join(flow_root, scene, '*.flo')))[args["weather_from_frame"]:args["weather_from_frame"]+no_imges]
