...
def get_loss(f_type, pred, target):
    ...
    if f_type == "aee":
        ...
        raise (
            NotImplementedError,
            "The requested loss type %s does not exist. Please choose one of 'aee', 'mse' or 'cosim'"
            % (f_type),
        )
...
def loss_delta_constraint(
    pred, target, delta1, delta2, device, delta_bound=0.001, mu=100.0, f_type="aee"
):
...
def loss_weighted(pred, target, delta1, delta2, c=1.0, f_type="aee"):
