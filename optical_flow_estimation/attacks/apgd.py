...
def apgd(attack_args: Dict[str, List[object]], inputs: Dict[str, torch.Tensor], model: BaseModel, targeted_inputs: Optional[Dict[str, torch.Tensor]]):
    ...
    if attack_args["attack_targeted"]:
        attack = APGD(model, eps=attack_args["attack_epsilon"], loss=attack_args["attack_loss"], verbose=False, steps=attack_args["apgd_steps"], n_restarts=attack_args["apgd_n_restarts"], seed=attack_args["apgd_seed"], rho=attack_args["apgd_rho"], eot_iter=attack_args["apgd_eot_iter"])
