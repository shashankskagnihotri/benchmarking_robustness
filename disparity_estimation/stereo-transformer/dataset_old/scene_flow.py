...
class SceneFlowSamplePackDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        occ = np.array(Image.open(os.path.join(path, self.occ_fold, self.data[idx]))).astype(bool)
        input_data['occ_mask'] = occ

        occ_right = np.array(Image.open(os.path.join(path, self.occ_fold_right, self.data[idx]))).astype(bool)
...
class SceneFlowMonkaaDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        occ_left = np.array(Image.open(occ_left_fname)).astype(bool)
        occ_right = np.array(Image.open(occ_right_fname)).astype(bool)
