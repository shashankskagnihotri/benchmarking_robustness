...
def corrupt(x, severity=1, corruption_name=None, corruption_number=-1):
    ...
    """
    if corruption_name:
         x_corrupted = corruption_dict[corruption_name](Image.fromarray(np.transpose(x.cpu().numpy(), (1, 2, 0))), severity)
    elif corruption_number != -1:
         x_corrupted = corruption_tuple[corruption_number](Image.fromarray(np.transpose(x.cpu().numpy(), (1, 2, 0))), severity)
    else:
         raise ValueError("Either corruption_name or corruption_number must be passed")

    return torch.from_numpy(np.transpose(np.uint8(x_corrupted),(2, 0, 1))).to(x.device)
