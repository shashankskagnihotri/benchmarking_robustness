    ...
    for corruption in corruptions:
        for severity in severities:
            ...
            output_dir = os.path.join(
                input_dir, "cc", corruption, f"severity_{severity}"
            )
...
def common_corruptions(
    ...
    for img_id in tqdm(
        cfg.img_ids, desc=f"Processing {cfg.corruption}, severity {cfg.severity}"
    ):
        if cfg.dataset_type == "coco":
        ...
        corrupted_image = corrupt(
            image, corruption_name=cfg.corruption, severity=cfg.severity
        )
...
if __name__ == "__main__":
    ...
    cfgs = get_corruptions_todo(
        dataset_type="coco", severities=severities, input_dir="data/coco/"
    )
    jobs += executor.map_array(common_corruptions, cfgs)
